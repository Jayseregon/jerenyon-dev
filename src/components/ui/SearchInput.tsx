  ...
  return (
    <div ref={searchInputRef} className="relative">
      {isSearchExpanded || alwaysExpanded ? (
        <div className={`relative ${isInsideNavbar ? "h-full" : "h-8"}`}>
          <input
            aria-label="Search"
            ...
            type="search"
          />
          <SearchIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 text-base text-primary pointer-events-none" />
        </div>
      ) : (
        <SearchIcon
